...
const isAnagram = (wordOne, wordTwo) => {
	return wordOne.toLowerCase() === wordTwo.toLowerCase()
		? false
		: wordOne.toLowerCase().split('').sort().join('') ===
		  wordTwo.toLowerCase().split('').sort().join('')
		? true
		: false;
};

console.log(isAnagram('roma', 'amor'));
console.log(isAnagram('frase', 'fresa'));
console.log(isAnagram('escandalizar', 'zascandilear'));
